...
Completed — WKWebView + 真 github-markdown-css 的 markdown 渲染已交付、用户实机验收通过（浅/深两外观、选中复制、大纲跳转、任务列表/表格/嵌套引用/标题底线像素级对齐 github）。W1/W2/W3 已合。后续跟踪进 issue：代码块语法高亮 → #16（仅知名语言时高亮）；原生 markdown 死路径清理 → #17。
