...
批次 2 补丁第二轮（2026-07-16 实机复验，架构师亲修）：① inline 胶囊在表格单元格里被 `boundingRect(forGlyphRange:in:)` 撑成整行单元格宽、且暗色下动态色按默认浅色解析成近白——改用 `enumerateEnclosingRects` 取字形紧致矩形，新增 `resolvedBackgroundColor` 按 textView `effectiveAppearance` 解析底色（块级/胶囊两分支共用）。② 用户选择「填满窗口去限宽」：`applyMarkdownReadingColumn` 改为固定对称内边距（markdown 32pt / 其余 18pt），正文靠 widthTracksTextView+autoresize 随窗口变宽而变宽；固定内边距不依赖当前宽度，初始 CLI 打开与拖拽 resize 都无需重算，故 V3 的 clip-view frameDidChange 观察者随之移除（连同 deinit/windowDidResize 中相关代码）。全量 128 测试全绿、release 构建通过；实机视觉待报告者复验。
