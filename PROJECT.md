...
PreviewWindowController(~1.8k 行，唯一有状态 UI 控制器)
  ...
  → WKWebView(markdown 预览) / PreviewGutterView(NSRulerView) / JSONOutlineView / FileTreeView / Drop*View / FileKind
...
叶子纯函数模块：JSONFormatter · JSONTreeIndex · XMLFormatter · MarkdownHTMLRenderer · MarkdownRenderer
               · SyntaxHighlighter · RepoRoot · DirectoryLister
服务单例：HighlightService（JSC + shiki-bundle，私有串行队列，故障永久降级不崩溃）
```

## 关键决策记录
- 非 markdown 渲染走 `NSTextView` + `NSAttributedString`；markdown 预览走 `WKWebView` + 内嵌真 github-markdown-css（像素级对齐 github，选中复制走 WebView 原生）；JSON/JSONL 默认视图为 NSOutlineView 树（2026-07-15 用户裁定），原文文本模式可切换。
