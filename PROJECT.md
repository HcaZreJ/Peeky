...
| 顶栏 4 个 Copy 按钮：全文（⌥⌘C）、文件名、绝对路径（⇧⌘C）、相对仓库根路径（⌥⇧⌘C；无仓库时置灰）；Reveal in Finder；⋯ 溢出菜单包含 Wrap Lines 开关；选区触发浮动 chip（`NSTextView` 与 Markdown WebView 两条路径，Markdown 场景通过源行号启发式定位）——"path:line" 恒有，JSON / JSONL 且路径非根时左侧并排 "jq path"，两者成组右对齐选区尾端后整体做越界校正 | ✅ |
