...
| 顶栏 4 个 Copy 按钮：全文（⌥⌘C）、文件名、绝对路径（⇧⌘C）、相对仓库根路径（⌥⇧⌘C；无仓库时置灰）；Reveal in Finder；⋯ 溢出菜单包含 Wrap Lines 开关；选区触发浮动 "Copy path:line" 按钮（`NSTextView` 与 Markdown WebView 两条路径，Markdown 场景通过源行号启发式定位） | ✅ |
...
- `RenderedPreview { attributedText, note, outline, display, highlightLanguage?, usesJSONHighlighting, followsSystemAppearance }`：`PreviewRenderer` 的输出。`highlightLanguage` 非 nil 表示走 shiki 高亮（Dark Modern）；`usesJSONHighlighting` 为 true 表示 JSON / JSONL 走可视区惰性词法高亮；`followsSystemAppearance` 为 true 表示编辑器区通过 `PeekyTheme` 跟随系统明暗
